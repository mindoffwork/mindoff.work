...
If you've worked with SQL, this is a `JOIN`. If you haven't: for each row in the left table, find the row in the right table where the key matches and stitch them together. The default is an inner join, which keeps only rows with matches on both sides, which is usually what you want for lookups.
...
Sometimes you need a breakdown by category as separate columns rather than a summed total, particularly useful when different categories drive different downstream logic. A loop would build a dictionary per order. Pivot does it in one operation.
...
## The five operations, assembled

With merge, groupby/aggregate, pivot, column math, and filter, the full order calculation runs without a single Python loop over rows:
